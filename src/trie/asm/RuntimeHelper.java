...
@SuppressWarnings({ "java:S112", "java:S3655", "java:S1181" })
final class RuntimeHelper {
	...
	private static class VarargsInvoker {
		...
		static Class<?> carrier(MemoryLayout layout, boolean ret) {
			if (layout instanceof ValueLayout valueLayout) {
				return (ret || valueLayout.carrier() != MemorySegment.class) ? valueLayout.carrier() : MemorySegment.class;
	...
	static final SegmentAllocator CONSTANT_ALLOCATOR = (size, align) -> MemorySegment.allocateNative(size, align, Arena.openShared().scope());
	...
	static final MethodHandle downcallHandle(String name, FunctionDescriptor fdesc, boolean variadic) {
		return SYMBOL_LOOKUP.find(name).map(
				addr -> (variadic ? VarargsInvoker.make(addr, fdesc) : LINKER.downcallHandle(addr, fdesc))).orElse(null);
	}

	static final MemorySegment lookupGlobalVariable(String name, MemoryLayout layout) {
		return SYMBOL_LOOKUP.find(name).map(symbol -> MemorySegment.ofAddress(symbol.address(), layout.byteSize(), Arena.openShared().scope())).orElse(null);
	}
	...
	static final <Z> MemorySegment upcallStub(Class<Z> fi, Z z, FunctionDescriptor fdesc, String mtypeDesc, SegmentScope scope) {
		try {
